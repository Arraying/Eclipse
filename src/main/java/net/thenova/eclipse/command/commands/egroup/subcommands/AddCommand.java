 ...
 */
public final class AddCommand extends AbstractSub {
    ...
    protected void handle(Arguments arguments) {
        ...
        if(Eclipse.getInstance().getPermissions().getGroup(group) != null) {
            context.reply(new MessageResponse("The group provided already exists."));
            return;
        }
        Eclipse.getInstance().getPermissions().createGroup(group);
        context.reply(new MessageResponse("The group has been created."));
    }
